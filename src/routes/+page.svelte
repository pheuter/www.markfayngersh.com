<div class="flex min-h-dvh flex-col items-center justify-center px-4 md:px-8">
	<div class="flex flex-col items-center gap-8 md:flex-row md:gap-16">
		<img src="/img/mark_outdoors.jpg" alt="Mark outdoors" class="size-80" />
		<div class="space-y-6 text-center md:text-left">
			<h1 class="font-serif text-5xl font-semibold">Mark Fayngersh</h1>
			<p class="max-w-md leading-relaxed">
				<strong>Mark</strong> is the cofounder of Careswitch, an AI-powered home care startup. He previously
				worked on a collaborative care platform at Quartet Health and researched interactions for large,
				distributed audiences at the MIT Media Lab.
			</p>
			<div class="flex items-center justify-center gap-6 md:justify-start">
				<a href="/posts" class="font-medium underline">Blog</a>
				<a href="/bookshelf" class="font-medium underline">Bookshelf</a>
				<a href="/projects" class="font-medium underline">Projects</a>
			</div>
		</div>
	</div>
</div>
